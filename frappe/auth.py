...
class HTTPRequest:
    ...
    def set_request_ip(self):
        if frappe.get_request_header("X-Forwarded-For"):
            frappe.local.request_ip = (
                frappe.get_request_header("X-Forwarded-For").split(
                    ",", 1)[0]).strip()
    ...
    def validate_csrf_token(self):
        if (not frappe.request
                ...
                or frappe.conf.ignore_csrf or not frappe.session
                or not (saved_token := frappe.session.data.csrf_token) or
            ((frappe.get_request_header("X-Frappe-CSRF-Token")
              or frappe.form_dict.pop("csrf_token", None)) == saved_token)):
            return
...
class LoginManager:
    ...
    def __init__(self):
        ...
        if frappe.local.form_dict.get(
                "cmd"
        ) == "login" or frappe.local.request.path == "/api/method/login":
    ...
    def login(self):
        if frappe.get_system_settings("disable_user_pass_login"):
            frappe.throw(_("Login with username and password is not allowed."),
    ...
    def get_user_info(self):
        self.info = frappe.get_cached_value(
            ...
            self.user,
            [
                "user_type", "first_name", "last_name", "user_image",
                "default_workspace"
            ],
            as_dict=1,
        )
    ...
    def set_user_info(self, resume=False):
        ...
        self.full_name = " ".join(
            filter(None, [self.info.first_name, self.info.last_name]))
        ...
        if self.info.user_type == "Website User":
            ...
            if not resume:
                ...
                if default_workspace:
                    frappe.local.response["home_page"] = "/app/" + slug(
                        default_workspace)
                else:
                    frappe.local.response["home_page"] = get_default_path(
                    ) or "/app"
        ...
        if not resume and (redirect_to := frappe.cache.hget(
                "redirect_after_login", self.user)):
    ...
    def authenticate(self, user: str | None = None, pwd: str | None = None):
        ...
        if len(pwd) > MAX_PASSWORD_SIZE:
            self.fail(_("Password size exceeded the maximum allowed size"),
                      user=user)
        ...
        ignore_tracker = should_run_2fa(user.name) and ("otp"
                                                        in frappe.form_dict)
        user_tracker = None if ignore_tracker else get_login_attempt_tracker(
            user.name)
    ...
    def force_user_to_reset_password(self):
        ...
        if reset_pwd_after_days:
            last_password_reset_date = (frappe.db.get_value(
                "User", self.user, "last_password_reset_date") or today())
    ...
    def validate_hour(self):
        ...
        login_before = int(
            frappe.db.get_value("User", self.user, "login_before", ignore=True)
            or 0)
        login_after = int(
            frappe.db.get_value("User", self.user, "login_after", ignore=True)
            or 0)
...
def validate_ip_address(user):
    ...
    if hasattr(frappe.local,
               "request") and frappe.local.request.path.startswith(
    ...
    user_info = (frappe.get_cached_value(
        "User", user, user_fields, as_dict=True)
                 if not frappe.flags.in_test else frappe.db.get_value(
                     "User", user, user_fields, as_dict=True))
    ...
    system_settings = (frappe.get_cached_doc("System Settings")
                       if not frappe.flags.in_test else
    ...
    frappe.throw(
        _("Access not allowed from this IP Address") +
        f": {frappe.local.request_ip}",
        frappe.AuthenticationError,
    )
...
def get_login_attempt_tracker(key: str, raise_locked_exception: bool = True):
    ...
    if track_login_attempts:
        ...
        tracker_kwargs[
            "max_consecutive_login_attempts"] = sys_settings.allow_consecutive_login_attempts
    ...
    if raise_locked_exception and track_login_attempts and not tracker.is_user_allowed(
        ...
        frappe.throw(
            _("Your account has been locked and will resume after {0} seconds"
              ).format(sys_settings.allow_login_after_fail),
...
class LoginAttemptTracker:
    ...
    def __init__(
        ...
        *,
        ...
        if user_name:
            deprecation_warning(
                "`username` parameter is deprecated, use `key` instead.")
...
def validate_api_key_secret(api_key,
                            api_secret,
                            frappe_authorization_source=None):
    """frappe_authorization_source to provide api key and secret for a doctype apart from User"""
    doctype = frappe_authorization_source or "User"
    docname = frappe.db.get_value(doctype=doctype,
                                  filters={
                                      "api_key": api_key,
                                      "enabled": True
                                  },
    ...
    if not docname:
        raise frappe.AuthenticationError
    form_dict = frappe.local.form_dict
    doc_secret = get_decrypted_password(doctype,
                                        docname,
                                        fieldname="api_secret")
    if api_secret == doc_secret:
        if doctype == "User":
            user = frappe.db.get_value(doctype="User",
